...
function RejectCoach({ user_id, handleUploadSuccessChange, firstName }) {
  ...
  const handleRejectCoach = async () => {
    ...
    //   if (!response.ok) {
    //     throw new Error(`Failed to reject coach. Status: ${response.status}`);
    //   }
    ...
    <>
      ...
      <Modal show={show} onHide={handleClose} backdrop="static" keyboard={false} centered>
        <Modal.Header className="text-center" closeButton>
          <Modal.Title className="w-100">Reject Coach</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <center>
            Are you sure you want reject coach "{firstName}"? <p />
            <b>This action cannot be undone.</b>
          </center>
          <button className="btn btn-danger w-100 mt-2" onClick={handleRejectCoach}>
            Reject
          </button>
        </Modal.Body>
      </Modal>
